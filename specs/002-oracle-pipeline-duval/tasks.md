...
- [ ] T001 [P] [SETUP] Create monorepo root with `package.json`, `tsconfig.json`, `tsconfig.base.json`, `.nvmrc` (Node 22.18), `.prettierrc`, `.eslintrc.cjs` at `oracle-property-intelligence-platform-pipeline-duval-fl/package.json`
- [ ] T002 [P] [SETUP] Create `oracle-property-intelligence-platform-pipeline-duval-fl/docker-compose.yml` with Restate 1.7 (ports 8080, 9070), Postgres 16 (port 5432, volume at /data/postgres), pipeline services (port 9080)
- [ ] T003 [P] [SETUP] Create CDK app entry at `oracle-property-intelligence-platform-pipeline-duval-fl/infra/bin/app.ts` that instantiates PipelineStack, FrontendStack, and AgentStack
- [ ] T004 [P] [SETUP] Create `oracle-property-intelligence-platform-pipeline-duval-fl/infra/lib/pipeline-stack.ts` — EC2 t3.large (us-east-2), security group (443 HTTPS, 22 SSH), 100GB gp3 EBS volume, IAM role, user-data script that installs Docker, runs `docker compose up -d`, configures Nginx reverse proxy with Caddy/certbot for HTTPS (`/api/*` -> :9080, `/restate` -> :9070)
- [ ] T005 [P] [SETUP] Create `oracle-property-intelligence-platform-pipeline-duval-fl/infra/lib/frontend-stack.ts` — Amplify app connected to delivery repo, auto-build React frontend on push, custom domain configuration, environment variables for API base URL
- [ ] T006 [P] [SETUP] Create `oracle-property-intelligence-platform-pipeline-duval-fl/infra/lib/agent-stack.ts` — Lambda function for Vercel AI SDK agent (DuckDB + httpfs), Lambda function for MCP server, API Gateway with routes `/agent` and `/mcp`, IAM roles for DuckDB httpfs access to Filebase
- [ ] T007 [SETUP] Create `oracle-property-intelligence-platform-pipeline-duval-fl/infra/package.json` and `oracle-property-intelligence-platform-pipeline-duval-fl/infra/tsconfig.json` with CDK dependencies (`aws-cdk-lib`, `constructs`, `@aws-cdk/aws-amplify-alpha`)
- [ ] T008 [SETUP] Create `oracle-property-intelligence-platform-pipeline-duval-fl/.env.example` with all required env vars: `FILEBASE_ACCESS_KEY`, `FILEBASE_SECRET_KEY`, `FILEBASE_BUCKET_OPEN_DATA`, `FILEBASE_BUCKET_QUERY_TABLE`, `WEBHOOK_URLS`, `WEBHOOK_SECRET`, `DATABASE_URL`, `RESTATE_RUNTIME_ENDPOINT`, AWS credentials
- [ ] T009 [SETUP] Scaffold empty directory structure for pipeline (`pipeline/src/workflows/`, `pipeline/src/services/`, `pipeline/src/transforms/duval/`, `pipeline/src/sources/`, `pipeline/src/lib/`, `pipeline/data/seeds/`, `pipeline/tests/unit/`, `pipeline/tests/integration/`), frontend (`frontend/src/pages/`, `frontend/src/components/`, `frontend/src/services/`), agent (`agent/src/tools/`, `agent/tests/`), mcp (`mcp/src/`, `mcp/tests/`) inside `oracle-property-intelligence-platform-pipeline-duval-fl/`
- [ ] T010 [SETUP] Deploy all CDK stacks — run `npx cdk deploy --all` to provision EC2 + Amplify + Lambda. Verify: EC2 instance is running, Docker Compose services (Restate + Postgres) are healthy, HTTPS endpoint responds, Amplify app is created
...
- [ ] T011 [FOUND] Implement `oracle-property-intelligence-platform-pipeline-duval-fl/pipeline/src/lib/db.ts` — Postgres connection pool using `pg` or `postgres` package, connection from `DATABASE_URL` env var, typed query helpers, migration runner
...
- [ ] T069 [POLISH] Run full end-to-end validation per quickstart.md — execute all 7 validation scenarios: (1) two consecutive ingestion runs showing incremental deltas, (2) IPFS publish with CID verification, (3) webhook delivery to test endpoint, (4) all 6 property search queries in UI, (5) agent chat multi-attribute query, (6) MCP client connection and query, (7) pipeline run history in UI with 2+ runs visible. Document results
